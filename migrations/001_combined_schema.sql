-- Combined migration (replaces 001_initial_schema, 002_add_peer_ip_to_computer_networks, and 004_rename_password_to_pin)

-- Create the networks table
CREATE TABLE IF NOT EXISTS networks (
  id VARCHAR(64) PRIMARY KEY,
  name VARCHAR(255) NOT NULL,
  pin VARCHAR(10) NOT NULL, -- Renamed from 'password' to 'pin'
...
-- Create unique index on owner_public_key to ensure each public key has only one network
CREATE UNIQUE INDEX IF NOT EXISTS idx_networks_owner_public_key ON networks(owner_public_key);
...
COMMENT ON COLUMN networks.pin IS 'PIN code to access the network'; -- Updated comment for renamed column
COMMENT ON COLUMN networks.owner_public_key IS 'Base64-encoded public key of the network creator';
...
CREATE TABLE IF NOT EXISTS computer_networks (
  ...
  peer_ip VARCHAR(15), -- Added from 002_add_peer_ip_to_computer_networks
  UNIQUE(network_id, public_key),
  UNIQUE(network_id, peer_ip), -- Added from 002_add_peer_ip_to_computer_networks
...
COMMENT ON COLUMN computer_networks.peer_ip IS 'The IP address assigned to this computer within the network';
